function result = FigureA_PCFC_improved(T_sys, P_sys)
% ==========================================================
% Figure A PCFC system model
% Input:
%   T_sys [K]
%   P_sys [Pa]
% Output (struct):
...
% Inlet state (13)
set(gas,'T',T(13),'P',P(13),'X',X(:,13));
h13 = enthalpy_mass(gas);      % J/kg
s13 = entropy_mass(gas);       % J/kg-K
T13 = T(13);

% ---- solve T2s such that s(T2s,P2,X)=s1 ----
T13_outs = solve_T_given_sP(gas, s13, P_out_exp, X(:,13), T13);

set(gas,'T',T13_outs,'P',P_out_exp,'X',X(:,13));
h13s_out = enthalpy_mass(gas);

% actual outlet enthalpy
h13_out  = h13 - eta_turb*(h13 - h13s_out);

% actual outlet temperature
set(gas,'P',P_out_exp,'H',h13_out,'X',X(:,13));
T13_out  = temperature(gas);

W_exp = mDot(13) * (h13 - h13_out);     % W
W_exp = max(W_exp,0);
...
W_comp = nDot(9) * R * T_amb * log(P(10)/P(9)) - W_exp;  % [W]
...
function T_sol = solve_T_given_sP(gas, s_target, P_target, X, T_guess)

    % bracket search (downward) to find sign change
    f = @(T) s_at(gas,T,P_target,X) - s_target;

    T_hi = max(T_guess, 300);   % start around inlet
    T_lo = max(5, 0.1*T_hi);    % go down
    ...
    if f_hi*f_lo > 0
        error("Failed to bracket root for T(s,P). Check state/units.");
    end

    T_sol = fzero(f, [T_lo, T_hi]);

end

function s = s_at(gas, T, P, X)
    set(gas,'T',T,'P',P,'X',X);
    s = entropy_mass(gas);
end

% ---------- 결과만 struct로 정리 ----------
result.eta_PCFC  = eta_PCFC;
result.W_PCFC     = W_PCFC;
result.W_comp     = W_comp;
result.Q_dot_PCFC = Q_dot_PCFC;
result.Temperature = T;
result.MassFlowRate = mDot;
result.composition = X;
result.valid      = true;
