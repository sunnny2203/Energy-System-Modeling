...
[T(15), T(18), Q_dot1B] = HEX(gas, ...
                T(14), P(14), mDot(14), X(:,14), ...   % Cold side
                T(17), P(17), mDot(17), X(:,17), ...   % Hot side
                eps_HX, P_sys);
set(gas, 'T', T(15), 'P', P(15), 'X', X(:,15));
h15=enthalpy_mass(gas);

set(gas, 'T', T(18), 'P', P(18), 'X', X(:,18));
h18=enthalpy_mass(gas);
%% Stream 19 & 20
...
% 이 stream은 HX3/cond 계산 후 결정됨.
% 일단 초기 guess를 두지만, 아래에서 overwrite된다.
nN2_28=0; nH2_28=0;
...
fprintf("\n여기에 나오는 residual은 고려안해준 Heat Transfer 일듯?\n");

% Water Condenser
H_in  = mDot(18)*h18;
H_out = mDot(19)*h19 + mDot(20)*h20;

res = H_out - H_in; % Qdot_wc
...
% HEX1
H_in  = mDot(14)*h14 + mDot(17)*h17;
H_out = mDot(15)*h15 + mDot(18)*h18;
...
H_in  = mDot(15)*h15;
H_out = mDot(16)*h16;
Q_in  = W_heater;   % heater supplies heat
...
% PCEC
H_in  = mDot(16)*h16 + mDot(23)*h23;
H_out = mDot(17)*h17 + mDot(24)*h24;

Q_in = Q_dot_PCEC; % Q_in - Q_out
W_in = W_PCEC;

res = H_out - (H_in + Q_in + W_in); % Q_in = Q_in - Q_out
...
% HEX2 
H_in  = mDot(22)*h22 + mDot(24)*h24;
H_out = mDot(23)*h23 + mDot(25)*h25;
...
% HEX3
H_in  = mDot(27)*h27 + mDot(25)*h25;
H_out = mDot(28)*h28 + mDot(26)*h26;
...
% Overall System
H_in  = mDot(14)*h14 + mDot(21)*h21;
H_out = mDot(19)*h19 + mDot(20)*h20 + mDot(29)*h29;

Q_in = W_heater + Q_dot_PCEC;
W_in = W_PCEC;

res = H_out - (H_in + Q_in - W_in);
